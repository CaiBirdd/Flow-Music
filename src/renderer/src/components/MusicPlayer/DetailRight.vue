...
interface Props {
  currentSong: GetMusicDetailData // 为了获取总时长 (dt)
...
<template>
  <div class="right">
    <div style="display: flex">
      <div v-if="props.currentSong.ar" class="current-time">
        {{ formattingTime(props.currentTime * 1000) }}
      </div>
      <!-- 分隔符 "/" -->
      <span style="margin: 0 5px; line-height: 15px">/</span>
      <div v-if="props.currentSong.ar" class="total-time">
        {{ formattingTime(props.currentSong.dt) }}
      </div>
    </div>
